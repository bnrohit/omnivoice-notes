...
function summaryHtml(s){const list=(title,arr)=>arr?.length?`<h4>${escapeHtml(title)}</h4><ul>${arr.map(x=>`<li>${escapeHtml(typeof x==='string'?x:x.task||'')}</li>`).join('')}</ul>`:'';return `<h3>${escapeHtml(s.headline||'Analysis')}</h3><p>${escapeHtml(s.summary||'')}</p>${list('Key points',s.keyPoints)}${list('Decisions',s.decisions)}${list('Action items',s.actionItems)}${list('Follow-ups',s.followUps)}${list('Topics',s.topics)}${s.sentiment?.label?`<h4>Sentiment</h4><p>${escapeHtml(s.sentiment.label)}${s.sentiment.explanation?` — ${escapeHtml(s.sentiment.explanation)}`:''}</p>`:''}${list('Questions',s.questions)}${s.flashcards?.length?`<h4>Flashcards</h4><ul>${s.flashcards.slice(0,10).map(f=>`<li><strong>${escapeHtml(f.front)}</strong> — ${escapeHtml(f.back)}</li>`).join('')}</ul>`:''}`}
